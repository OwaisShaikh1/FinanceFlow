...
// --------------------- Auth Utilities ---------------------
const JWT_SECRET = 'supersecretkey'; // in production, use .env

const signToken = (user) =>
  jwt.sign({ id: user.id, role: user.role, biz: user.business }, JWT_SECRET, { expiresIn: '7d' });
...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    users.push(user);
    return res.status(201).json({ id: user.id, message: "User registered successfully" });
  } catch (e) {
    return res.status(400).json({ message: e.message });
  }
});
